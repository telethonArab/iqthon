...
- CH: @iqthon
  """,
    ...
    await event.delete()
# WRITE ON GIFs
async def WriteOnGifs(GifPath, text):
    read_gif = Image.open(GifPath)
...
async def GetGifs(event):
    if event.reply_to != None:
        ...
        try:
            if message_media.messages[0].media != None and message_media.messages[0].media.document.mime_type == 'video/mp4':
                edit = await event.edit('**في طور المعالجة و الانجاز**')
                path = await event.client.download_media(message_media.messages[0], 'video_to_edit')
                
                # read the video and convert
                video = VideoFileClip(path)
                gif_path = video.write_gif('converted.gif', fps=10)
                
                text = (event.message.message).replace('.كتابة متحركة', '').strip()
                write_on = await WriteOnGifs('converted.gif', text)
                send_new_gif = await event.client.send_file(entity='me', file=write_on, force_document=True)
                edit = await event.edit('**تم الانجاز**')
...
# PRO USER MESSAGE - DELETE MESSAGE - ACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.اغلاق البرايم بالحظر'))
async def ProMessageDeleteActive(event):
    global ProUsersBanChannels
    
    ProUsersBanList.clear()
    if event.chat_id not in ProUsersBanChannels:
        ProUsersBanChannels.append(event.chat_id)
        
    edit = await event.edit('**تم تفعيل اغلاق البرايم بالحظر هنا**')


# PRO USER MESSAGE - DELETE MESSAGE - UNACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.فتح البرايم بالحظر ?(.*)'))
async def ProMessageDeleteActive(event):
    global ProUsersBanList, ProUsersBanChannels
    
    user_id = (event.message.message).replace('.فتح البرايم بالحظر', '').strip()
    if user_id == '':
        if event.chat_id in ProUsersBanChannels:
            ProUsersBanChannels.remove(event.chat_id)
        edit = await event.edit('**تم فتح البرايم بالحظر هنا**')
    else:
        try:
            user = await event.client(GetFullUserRequest(id=int(user_id)))
        except:
            user = await event.client(GetFullUserRequest(id=str(user_id)))
        if event.chat_id in ProUsersBanList:
            if user.full_user.id not in ProUsersBanList[event.chat_id]:
                ProUsersBanList[event.chat_id].append(user.full_user.id)
                
        unban = await event.client(EditBannedRequest(
            channel=event.chat_id,
            participant=user.full_user.id,
            banned_rights=ChatBannedRights(
                until_date=None,
                view_messages=None
            )
        ))
        edit = await event.edit('**تم فتح البرايم بالحظر على المستخدم**')
    

# PRO USER MESSAGE - DELETE MESSAGE - DELETE STATUS
@iqthon.on(events.NewMessage)
async def ProMessageDeleteWatch(event):
    global ProUsersBanList, ProUsersBanChannels

    if event.chat_id in ProUsersBanChannels:
        if event.chat_id not in ProUsersBanList:
            ProUsersBanList[event.chat_id] = []
            
        if event.message.entities != None:
            for item in event.message.entities:
                if str(item).startswith('MessageEntityCustomEmoji'):
                    if event.message.peer_id.user_id not in ProUsersBanList[event.chat_id]:
                        delete = await event.client.delete_messages(entity=event.chat_id, message_ids=[event.message.id], revoke=True)
                        Block = await event.client(EditBannedRequest(
                            channel=event.chat_id, 
                            participant=event.message.peer_id.user_id, 
                            banned_rights=ChatBannedRights(
                                until_date=None,
                                view_messages=True,
                                send_messages=True
...
ProUsersMulteChannels = []
ProUsersMulteList = {}
ProUsersMulted = []


# PRO USER MESSAGE - DELETE MESSAGE - ACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.اغلاق البرايم بالكتم'))
async def ProMessageDeleteActive(event):
    global ProUsersMulteChannels
    
    ProUsersMulteList.clear()
    if event.chat_id not in ProUsersMulteChannels:
        ProUsersMulteChannels.append(event.chat_id)
        
    edit = await event.edit('**تم تفعيل اغلاق البرايم بالكتم هنا**')


# PRO USER MESSAGE - DELETE MESSAGE - UNACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.فتح البرايم بالكتم ?(.*)'))
async def ProMessageDeleteActive(event):
    global ProUsersMulteList, ProUsersMulteChannels
    
    user_id = (event.message.message).replace('.فتح البرايم بالكتم', '').strip()
    if user_id == '':
        if event.chat_id in ProUsersMulteChannels:
            ProUsersMulteChannels.remove(event.chat_id)
        edit = await event.edit('**تم فتح البرايم بالكتم هتا**')
    else:
        try:
            user = await event.client(GetFullUserRequest(id=int(user_id)))
        except:
            user = await event.client(GetFullUserRequest(id=str(user_id)))
        if event.chat_id in ProUsersMulteList:
            if user.full_user.id not in ProUsersMulteList[event.chat_id]:
                ProUsersMulteList[event.chat_id].append(user.full_user.id)
                
        if user.full_user.id in ProUsersMulted:
            multe = await event.client.edit_permissions(event.chat_id, user.full_user.id, send_messages=False)
            ProUsersMulted.remove(user.full_user.id)
        edit = await event.edit('**تم فتح البرايم بالكتم على المستخدم**')
    

# PRO USER MESSAGE - DELETE MESSAGE - CLEAR
@iqthon.on(events.NewMessage(outgoing=True, pattern='.مسح البرايم بالكتم'))
async def ProMessageDeleteActive(event):
    global ProUsersMulted
    
    for user_id in ProUsersMulted:
        multe = await event.client.edit_permissions(event.chat_id, user_id, send_messages=False)
        ProUsersMulted.remove(user_id)
    edit = await event.edit('**تم مسح المكتومين البرايم**')
    

# PRO USER MESSAGE - DELETE MESSAGE - DELETE STATUS
@iqthon.on(events.NewMessage)
async def ProMessageDeleteWatch(event):
    global ProUsersMulteList, ProUsersMulteChannels, ProUsersMulted

    if event.chat_id in ProUsersMulteChannels:
        if event.chat_id not in ProUsersMulteList:
            ProUsersMulteList[event.chat_id] = []
            
        if event.message.entities != None:
            for item in event.message.entities:
                if str(item).startswith('MessageEntityCustomEmoji'):
                    if event.message.peer_id.user_id not in ProUsersMulteList[event.chat_id]:
                        delete = await event.client.delete_messages(entity=event.chat_id, message_ids=[event.message.id], revoke=True)
                        multe = await event.client.edit_permissions(event.chat_id, event.message.peer_id.user_id, send_messages=None, until_date=None)
                        if event.message.peer_id.user_id not in ProUsersMulted:
                            ProUsersMulted.append(event.message.peer_id.user_id)
                                
                              
ProUsersDeleteChannels = []
ProUsersDeleteList = {}


# PRO USER MESSAGE - DELETE MESSAGE - ACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.اغلاق البرايم بالمسح'))
async def ProMessageDeleteActive(event):
    global ProUsersDeleteChannels
    
    ProUsersDeleteList.clear()
    if event.chat_id not in ProUsersDeleteChannels:
        ProUsersDeleteChannels.append(event.chat_id)
        
    edit = await event.edit('**تم تفعيل اغلاق البرايم بالمسح هنا**')


# PRO USER MESSAGE - DELETE MESSAGE - UNACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.فتح البرايم بالمسح ?(.*)'))
async def ProMessageDeleteActive(event):
    global ProUsersDeleteList, ProUsersDeleteChannels
    
    user_id = (event.message.message).replace('.فتح البرايم بالمسح', '').strip()
    if user_id == '':
        if event.chat_id in ProUsersDeleteChannels:
            ProUsersDeleteChannels.remove(event.chat_id)
        edit = await event.edit('**تم فتح البرايم بالمسح هنا**')
    else:
        try:
            user = await event.client(GetFullUserRequest(id=int(user_id)))
        except:
            user = await event.client(GetFullUserRequest(id=str(user_id)))
        if event.chat_id in ProUsersDeleteList:
            if user.full_user.id not in ProUsersDeleteList[event.chat_id]:
                ProUsersDeleteList[event.chat_id].append(user.full_user.id)
        edit = await event.edit('**تم فتح البرايم بالمسح على المستخدم**')
...
async def ProMessageDeleteWatch(event):
    ...
    if event.chat_id in ProUsersDeleteChannels:
        if event.chat_id not in ProUsersDeleteList:
            ProUsersDeleteList[event.chat_id] = []
            
        if event.message.entities != None:
            for item in event.message.entities:
                if str(item).startswith('MessageEntityCustomEmoji'):
                    if event.message.peer_id.user_id not in ProUsersDeleteList[event.chat_id]:
                        delete = await event.client.delete_messages(entity=event.chat_id, message_ids=[event.message.id], revoke=True)
                                
ProUsersMulteProfileChannels = []
ProUsersMulteProfileList = {}
ProUsersProfileMulted = []


# PRO USER MESSAGE - DELETE MESSAGE - ACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.اغلاق مستخدمين البرايم بالكتم'))
async def ProMessageMulteProfileActive(event):
    global ProUsersMulteProfileChannels
    
    ProUsersMulteProfileList.clear()
    if event.chat_id not in ProUsersMulteProfileChannels:
        ProUsersMulteProfileChannels.append(event.chat_id)
        
    edit = await event.edit('**تم تفعيل .اغلاق مستخدمين البرايم بالكتم هنا**')


# PRO USER MESSAGE - DELETE MESSAGE - UNACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.فتح اغلاق مستخدمين البرايم بالكتم ?(.*)'))
async def ProMessageMulteProfileUnActive(event):
    global ProUsersMulteProfileList, ProUsersMulteProfileChannels
    
    user_id = (event.message.message).replace('.فتح اغلاق مستخدمين البرايم بالكتم', '').strip()
    if user_id == '':
        if event.chat_id in ProUsersMulteProfileChannels:
            ProUsersMulteProfileChannels.remove(event.chat_id)
        edit = await event.edit('**تم .فتح اغلاق مستخدمين البرايم بالكتم هنا**')
    else:
        try:
            user = await event.client(GetFullUserRequest(id=int(user_id)))
        except:
            user = await event.client(GetFullUserRequest(id=str(user_id)))
        if event.chat_id in ProUsersMulteProfileList:
            if user.full_user.id not in ProUsersMulteProfileList[event.chat_id]:
                ProUsersMulteProfileList[event.chat_id].append(user.full_user.id)
                
        if user.full_user.id in ProUsersProfileMulted:
            multe = await event.client.edit_permissions(event.chat_id, user.full_user.id, send_messages=False)
            ProUsersProfileMulted.remove(user.full_user.id)
        edit = await event.edit('**تم .فتح اغلاق مستخدمين البرايم بالكتم على المستخدم**')
    

# PRO USER MESSAGE - DELETE MESSAGE - CLEAR
@iqthon.on(events.NewMessage(outgoing=True, pattern='.مسح اغلاق مستخدمين البرايم بالكتم'))
async def ProMessageMulteProfileDelete(event):
    global ProUsersProfileMulted
    
    for user_id in ProUsersProfileMulted:
        multe = await event.client.edit_permissions(event.chat_id, user_id, send_messages=False)
        ProUsersProfileMulted.remove(user_id)
    edit = await event.edit('**تم مسح المكتومين مستخدمين البرايم*')
    

# PRO USER MESSAGE - DELETE MESSAGE - DELETE STATUS
@iqthon.on(events.NewMessage)
async def ProMessageMulteProfileWatch(event):
    global ProUsersMulteProfileList, ProUsersMulteProfileChannels, ProUsersProfileMulted

    if event.chat_id in ProUsersMulteProfileChannels:
        if event.chat_id not in ProUsersMulteProfileList:
            ProUsersMulteProfileList[event.chat_id] = []
            
        try:
            user_id = event.message.peer_id.user_id
            full_user = await event.client.get_entity(user_id)
            if full_user.premium != False:
                if user_id not in ProUsersMulteProfileList[event.chat_id]:
                    delete = await event.client.delete_messages(entity=event.chat_id, message_ids=[event.message.id], revoke=True)
                    multe = await event.client.edit_permissions(event.chat_id, user_id, send_messages=None, until_date=None)
                    if user_id not in ProUsersProfileMulted:
                        ProUsersProfileMulted.append(user_id)
        except:
            try:
...
ProUsersBanProfileChannels = []
ProUsersBanProfileList = {}


# PRO USER MESSAGE - DELETE MESSAGE - ACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.اغلاق مستخدمين البرايم بالحظر'))
async def ProMessageBanActive(event):
    global ProUsersBanProfileChannels
    
    ProUsersBanProfileList.clear()
    if event.chat_id not in ProUsersBanProfileChannels:
        ProUsersBanProfileChannels.append(event.chat_id)
        
    edit = await event.edit('**تم تفعيل .اغلاق مستخدمين البرايم بالحظر هنا**')


# PRO USER MESSAGE - DELETE MESSAGE - UNACTIVE
@iqthon.on(events.NewMessage(outgoing=True, pattern='.فتح اغلاق مستخدمين البرايم بالحظر ?(.*)'))
async def ProMessageBanActive(event):
    global ProUsersBanProfileList, ProUsersBanProfileChannels
    
    user_id = (event.message.message).replace('.فتح اغلاق مستخدمين البرايم بالحظر', '').strip()
    if user_id == '':
        if event.chat_id in ProUsersBanProfileChannels:
            ProUsersBanProfileChannels.remove(event.chat_id)
        edit = await event.edit('**تم فتح .اغلاق مستخدمين البرايم بالحظر هنا**')
    else:
        try:
            user = await event.client(GetFullUserRequest(id=int(user_id)))
        except:
            user = await event.client(GetFullUserRequest(id=str(user_id)))
        if event.chat_id in ProUsersBanProfileList:
            if user.full_user.id not in ProUsersBanProfileList[event.chat_id]:
                ProUsersBanProfileList[event.chat_id].append(user.full_user.id)
        ...
        edit = await event.edit('**تم فتح .اغلاق مستخدمين البرايم بالحظر على المستخدم**')
    

# PRO USER MESSAGE - DELETE MESSAGE - DELETE STATUS
@iqthon.on(events.NewMessage)
async def ProMessageBanWatch(event):
    global ProUsersBanProfileList, ProUsersBanProfileChannels

    if event.chat_id in ProUsersBanProfileChannels:
        if event.chat_id not in ProUsersBanProfileList:
            ProUsersBanProfileList[event.chat_id] = []
            
        try:
            user_id = event.message.peer_id.user_id
            full_user = await event.client.get_entity(user_id)
            if full_user.premium != False:
                if user_id not in ProUsersBanProfileList[event.chat_id]:
                    delete = await event.client.delete_messages(entity=event.chat_id, message_ids=[event.message.id], revoke=True)
                    Block = await event.client(EditBannedRequest(
                        channel=event.chat_id, 
                        participant=user_id, 
